...
def design_optimisation(predictive, candidate_designs, theta_samples,
    ...
    Args:
    predictive: Anonymous function of the form f(theta,D) that
        returns p(Y==1|theta,D), i.e. the predictive distribution of y for
        a bernoulli given a particular theta and D. MUST BE CORRECTLYNORMALIZED
    ...
    theta_samples: (nT x dT array) Set of unweighted samples representing
        ...
        proportional to the number of theta_samples therefore if things are
    ...
    assert candidate_designs.ndim == 2, "candidate_designs must be 2D matrix"

    nD = candidate_designs.shape[0]
    nT = theta_samples.shape[0]
    ...
    np.random.shuffle(theta_samples)
    ...
    for nSam in range(n_steps):
        ...
        if sum(U) == 0:
            ...
            chosen_design = candidate_designs[np.random.randint(0, nD), :]
            estimated_utilities = U
            return (chosen_design, estimated_utilities)

        pD = calculate_pD(U, gamma[nSam], pD_min)

        D_samples, iSamples = sample_rows(candidate_designs,
                                          size=n_particles,
                                          replace=True,
                                          p=pD)
        ...
        theta_iter, theta_pos_counter = get_theta_subset(theta_samples,
                                                         theta_pos_counter,
                                                         n_particles)

        # Call one step predictive function for each design-parameter pair,
        # note that this is already normalized so pnotY = 1-pY.
        p_y_given_theta_and_D = predictive(theta_iter, D_samples)
        log_p_y_given_theta_and_D = np.exp(p_y_given_theta_and_D)

        # Calculate p(Y|D) by marginalizing over theta
        p_y_given_D_iter_times_n_samples = np.bincount(iSamples,
                                                       weights=p_y_given_theta_and_D,
                                                       minlength=nD)
        ...
        U = calc_utility(U, p_y_given_theta_and_D, log_p_y_given_theta_and_D,
    ...
    chosen_design = candidate_designs[np.argmax(U), :]
...
def get_theta_subset(theta_samples, theta_pos_counter, n_particles):
    nT = theta_samples.shape[0]
    theta_end_position = theta_pos_counter+n_particles
    if theta_end_position < nT:
        idx = np.arange(theta_pos_counter, theta_end_position)
        theta_iter = theta_samples[idx, :]
    else:
        idx = np.concatenate([np.arange(0, np.mod(theta_end_position, nT)),
                              np.arange(theta_pos_counter, nT)])
        theta_iter = theta_samples[idx, :]

    theta_pos_counter = np.mod(theta_end_position, nT)
    return (theta_iter, theta_pos_counter)


def calc_utility(U, p_y_given_theta_and_D, log_p_y_given_theta_and_D,
                 p_y_given_D, iSamples, nD, n_times_sampled,
                 n_times_sampled_iter):
    ...
    U_theta = (p_y_given_theta_and_D *
               (log_p_y_given_theta_and_D -
                np.log(p_y_given_D[iSamples])) +
               (1-p_y_given_theta_and_D) *
               (np.log(1-p_y_given_theta_and_D) -
                np.log(1-p_y_given_D[iSamples])))
    ...
    U_theta = np.maximum(U_theta, 0)

    # Final utility from iterations also marginalized out over theta
    U_iter_times_n_samples = np.bincount(iSamples, U_theta, minlength=nD)
